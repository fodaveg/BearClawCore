...
public class NoteHandler: NSObject, ObservableObject {
    @MainActor public static let shared = NoteHandler()
    ...
    @objc public func openHomeNote() {
        ...
        if !homeNoteID.isEmpty {
            ...
            if let url = URL(
                string:
                    "bear://x-callback-url/open-note?id=\(homeNoteID)&exclude_trashed=yes"
            ) {
                openURL(url)
            }
        } else {
            ...
            guard
                let homeTemplate = SettingsManager.shared.loadTemplates().first(
                    where: { $0.isHome })
            else { return }
    ...
    @objc public func syncCalendarForDate(_ date: String?) {
        ...
        let fetchURLString =
            "bear://x-callback-url/open-note?title=\(date)&show_window=no&open_note=no&x-success=fodabear://update-daily-note-if-needed-success-for-sync"
        if let fetchURL = URL(
            string: fetchURLString.addingPercentEncoding(
    ...
    @objc func syncNoteById(url: URL) {
            ...
            url: url, resolvingAgainstBaseURL: false)?.queryItems
        {
            ...
            let fetchURLString =
                "bear://x-callback-url/open-note?id=\(noteId ?? "")&show_window=no&open_note=no&x-success=fodabear://update-daily-note-if-needed-success-for-sync"
    ...
    @objc public func updateHomeNoteIfNeeded(id: String? = nil) {
        let homeNoteId = id ?? SettingsManager.shared.homeNoteID
        let fetchURLString =
            "bear://x-callback-url/open-note?id=\(homeNoteId)&show_window=no&open_note=no&x-success=fodabear://update-home-note-if-needed-success&x-error=fodabear://update-home-note-if-needed-error"
        if let fetchURL = URL(
            string: fetchURLString.addingPercentEncoding(
                withAllowedCharacters: .urlQueryAllowed) ?? "")
        {
    ...
    @objc public func updateDailyNoteIfNeeded(_ date: String?) {
        ...
        let fetchURLString =
            "bear://x-callback-url/open-note?title=\(date ?? currentDateFormatted)&show_window=no&open_note=no&x-success=fodabear://update-daily-note-if-needed-success"
        if let fetchURL = URL(
            string: fetchURLString.addingPercentEncoding(
                withAllowedCharacters: .urlQueryAllowed) ?? "")
        {
    ...
    @objc public func updateDailyNoteIfNeededSuccess(url: URL) {
        ...
        guard
            ...
            let id = queryItems.first(where: { $0.name == "identifier" })?.value
        else { return }

        DispatchQueue.main.async {
            NoteManager.shared.updateDailyNoteWithCalendarEvents(
                for: title, noteContent: note, noteId: id)
        }
    }
    ...
    @objc public func updateDailyNoteIfNeededSuccessForSync(url: URL) {
        ...
        DispatchQueue.main.async {
            NoteManager.shared.updateDailyNoteWithCalendarEvents(
                for: title, noteContent: note, noteId: id, open: false)
        }
    }
    ...
    @objc public func updateNoteAndOpen(url: URL) {
        ...
        guard let note = queryItems.first(where: { $0.name == "note" })?.value
        else { return }
        guard
            let id = queryItems.first(where: { $0.name == "identifier" })?.value
        else { return }

        if id == SettingsManager.shared.homeNoteID {
            NoteManager().updateDailyNoteWithCalendarEvents(
                for: getCurrentDateFormatted(), noteContent: note, noteId: id,
                open: true)

        } else {
            NoteManager().updateDailyNoteWithCalendarEvents(
                for: title, noteContent: note, noteId: id, open: true)
    ...
    @objc public func openNoteForNoteAndOpen(url: URL) {
        ...
        print("new note id = \(id)")
        let fetchURLString =
            "bear://x-callback-url/open-note?id=\(id)&show_window=no&open_note=no&x-success=fodabear://replace-sync-placeholder-action"
        if let fetchURL = URL(
            string: fetchURLString.addingPercentEncoding(
                withAllowedCharacters: .urlQueryAllowed) ?? "")
        {
    ...
    @objc public func openHomeNoteAndSaveId(url: URL) {
        guard
            let components = URLComponents(
                url: url, resolvingAgainstBaseURL: false),
            let queryItems = components.queryItems
        ...
        SettingsManager.shared.homeNoteID = id
        updateHomeNoteIfNeeded()
    ...
    @objc public func updateHomeNoteIfNeededSuccess(url: URL) {
        ...
        DispatchQueue.main.async {
            NoteManager.shared.updateHomeNoteWithCalendarEvents(
                for: currentDateFormatted, noteContent: note,
    ...
    @objc public func openDailyNoteForDate(url: URL) {
        ...
        guard let date = queryItems.first(where: { $0.name == "date" })?.value
        else { return }
        print("URL: \(url)")
    ...
    @objc public func openDailyNote() {
        ...
        if let dailyUrl = URL(
            string:
                "bear://x-callback-url/open-note?title=\(dateToday)&open_note=no&show_window=no&exclude_trashed=yes&x-success=\(successParameter)&x-error=\(errorParameter)"
        ) {
            openURL(dailyUrl)
        }
    }
    ...
    @objc public func openDailyNoteSuccess(url: URL) {
            ...
            url: url, resolvingAgainstBaseURL: false)?.queryItems
        {
            ...
            if let dailyUrl = URL(
                string:
                    "bear://x-callback-url/open-note?id=\(String(describing: dailyId))"
            ) {
                openURL(dailyUrl)
            }
        }
    ...
    @objc public func openDailyNoteWithDate(_ date: String?) {
        ...
        let errorParameter =
            "fodabear://create-daily-note-for-date?date=\(date)"
        updateDailyNoteIfNeeded(date)
        if let dailyUrl = URL(
            string:
                "bear://x-callback-url/open-note?title=\(date)&open_note=no&show_window=no&exclude_trashed=yes&x-success=\(successParameter.addingPercentEncodingForRFC3986() ?? "")&x-error=\(errorParameter.addingPercentEncodingForRFC3986() ?? "")"
        ) {
    ...
    @objc public func openDailyNoteWithDateError(url: URL) {
        if let queryItems = URLComponents(
            url: url, resolvingAgainstBaseURL: false)?.queryItems
        {
            let dailyDate = queryItems.first(where: { $0.name == "date" })?
                .value
    ...
    @objc public func createDailyNoteWithDateString(_ date: String?) {
        ...
        print(date)
        guard
            let template = SettingsManager.shared.loadTemplates().first(where: {
                $0.name == "Daily"
            })
        else { return }
        print(template)
    ...
    @objc public func getCurrentDateFormatted(date: Date = Date()) -> String {
        ...
        @objc public func openTemplateNote(_ sender: NSMenuItem) {
            ...
            let templateNameComponents = templateTitle.components(
                separatedBy: " ")

            guard templateNameComponents.count > 2 else { return }

            let templateName = templateNameComponents.dropFirst().dropLast()
                .joined(separator: " ")

            guard
                let template = SettingsManager.shared.loadTemplates().first(
                    where: { $0.name == templateName })
            else { return }
