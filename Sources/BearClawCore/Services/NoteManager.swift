...
public class NoteManager: ObservableObject {
    @MainActor public static let shared = NoteManager()
    ...
    public func createDailyNoteForDate(selectedDate: Date) {
        ...
        getDailyNoteID(for: dateString) { noteContent in
            if noteContent.isEmpty {
                self.templateManager.createDailyNoteWithTemplate(
                    for: dateString)
            } else {
    ...
    public func getDailyNoteID(
        for dateString: String, completion: @escaping (String) -> Void
    ) {
        let searchText = dateString

        let tag =
            UserDefaults.standard.string(forKey: "dailyNoteTag")?
            .addingPercentEncodingForRFC3986() ?? ""

        let searchURLString =
            "bear://x-callback-url/search?term=\(searchText)&tag=\(tag)&x-success=fodabear://open-note-success&x-error=fodabear://open-note-error&token=XXXXX"
    ...
    public func updateDailyNoteWithCalendarEvents(
        for dateString: String, noteContent: String, noteId: String,
        open: Bool = true
    ) {
        ...
        updatedContent = self.templateManager.replaceSyncSection(
            in: updatedContent, id: noteId)

        DispatchQueue.main.async {
            self.bearManager.updateNoteContent(
                newContent: updatedContent, noteID: noteId, open: open,
                show: open)
        }
    }

    public func updateHomeNoteWithCalendarEvents(
        for dateString: String, noteContent: String, homeNoteId: String
    ) {
        let selectedCalendars = calendarManager.selectedCalendars()
        let events = calendarManager.fetchCalendarEvents(
            for: dateString, calendars: selectedCalendars)
        ...
        let updatedContent = templateManager.replaceDailySection(
            in: noteContent, with: dateString)
        let fullyUpdatedContent = templateManager.replaceCalendarSection(
            in: updatedContent, with: cleanedEvents)
        bearManager.updateNoteContent(
            newContent: fullyUpdatedContent, noteID: homeNoteId, open: false,
            show: false)
    }
