...
public class SettingsManager: ObservableObject {
    ...
    public func setLaunchAtLogin(enabled: Bool) {
        ...
        let helperAppService = SMAppService.loginItem(identifier: helperAppIdentifier)
        ...
        do {
            if enabled {
                if status == .notRegistered || status == .notFound {
                    ...
                    print("Successfully set launch at login for \(helperAppIdentifier)")
                    sendNotification(title: "Login Item Enabled", body: "The application will now launch at login.")
                ...
                if status == .enabled {
                    ...
                    print("Successfully unset launch at login for \(helperAppIdentifier)")
                    sendNotification(title: "Login Item Disabled", body: "The application will no longer launch at login.")
                } else {
                    print("Launch at login is already disabled or not registered")
                }
            }
        } catch {
            print("Failed to update launch at login status for \(helperAppIdentifier): \(error.localizedDescription)")
        }
    }
